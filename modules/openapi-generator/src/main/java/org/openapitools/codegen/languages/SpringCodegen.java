...
public class SpringCodegen extends AbstractJavaCodegen
    ...
    protected void processGenerateRequiredVarConstructor(Map<String, CodegenModel> allModels) {
        for (CodegenModel cm : allModels.values()) {
            if (cm.vendorExtensions.containsKey("x-java-required-args-constructor")) {
                List<CodegenProperty> superVars = new ArrayList<>();
                List<CodegenProperty> requiredParentVars = new ArrayList<>();
                List<CodegenProperty> requiredVars = new ArrayList<>();
                cm.vendorExtensions.put("x-java-required-args-constructor-super", superVars);
                cm.vendorExtensions.put("x-java-required-args-constructor-parentVars", requiredParentVars);
                cm.vendorExtensions.put("x-java-required-args-constructor-vars", requiredVars);

                if (cm.parentModel == null) {
                    // no parent, just use the requiredVars
                    for (CodegenProperty property : cm.requiredVars) {
                        requiredVars.add(property);
                    }
                } else {
                    // when super constructor exist
                    // first check if the parentRequiredVars are an exact match of parentModel.requiredVars
                    boolean parentMatch = cm.parentModel != null &&
                            cm.parentRequiredVars.stream().map(CodegenProperty::getName).collect(Collectors.toList())
                                    .equals(cm.parentModel.requiredVars.stream().map(CodegenProperty::getName).collect(Collectors.toList()));
                    if (parentMatch) {
                        // use super required constructor
                        superVars.addAll(cm.parentRequiredVars);
                        for (CodegenProperty property : cm.requiredVars) {
                            if (!codeGenPropertyExistInCollection(property, cm.parentRequiredVars)) {
                                if (codeGenPropertyExistInCollection(property, cm.parentVars)) {
                                    requiredParentVars.add(property);
                                } else {
                                    requiredVars.add(property);
                                }
                            }
                        ...
                        // discrepency, use list of fluent setters
                        for (CodegenProperty property : cm.requiredVars) {
                            if (codeGenPropertyExistInCollection(property, cm.parentVars)) {
                                requiredParentVars.add(property);
                            } else {
    ...
    private static boolean codeGenPropertyExistInCollection(CodegenProperty property, List<CodegenProperty> collection) {
        for (CodegenProperty p: collection) {
